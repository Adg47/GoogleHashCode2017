...
class Routeur :
    ...
    def liste_couvertes(self):
        x = 0
        y = 0
        liste_cell = [] # Liste cellules couvertes par routeur
...
# --------------------------------
#Vérifie le score et enlève des routeurs si le budget est dépassé.

def verification_budget(budget):
    if budget < 0:
        nombre_cable = len(backbone)- 1 # On retire le backbone des cellules cables car prix différent
        backbone = [] # on la vide pour recommencer après avoir enlevé des routeurs
        backbone.append(backbone_coord)
        while (budget <(prix_routeur *100)):
            dernier_element = len(liste_routeurs)-1
            del liste_routeurs[dernier_element]
            budget += prix_routeur
        budget += nombre_cable*prix_backbone #On rajoute le prix des cables enlevés
        for i in len(liste_routeurs):
            backbone = liste_routeurs[i].cellule.relier()[0] # On met les coordonnées des cellules reliant le routeur au backbone
            budget -= prix_routeur + liste_routeurs[i].cellule.relier()[1] * prix_backbone # Budget s'autodécrémente
...
def Placer_routeur():
    ...
    while ( fin == False and budget > prix_routeur):
        ...
        if (budget - prix_routeur > 0) :
            # On choisit le routeur qui couvre le plus de cellule
            cell = plusfort()
            if (cell != None):
                # Si le relier au backbone est dans le budget
                cell_relier = cell.relier()[1]
                print('relier',cell_relier)
                if (budget - cell_relier * prix_backbone >= 0):
                    ...
                    budget -= prix_routeur + cell_relier * prix_backbone # Budget s'autodécrémente
...
budget = int(donnees[5]) + prix_routeur
