...
def liste_couvertes(cellrouteur):
    ...
    for a in liste_map :
        if dist(coord,a.coord) < rayon and elimination(a.coord,cellrouteur) == False and a.etat_wifi == False and a.type=='.' :  # a est la case évaluée, cellrouteur est le routeur considéré
            liste.append(a)
    return liste
# --------------------------------
...
def fichier_de_fin(backbone,liste_routeur):
    with open("C:\Documents\G12_rue_de_londres.out","a") as fichier:
...
# ===== Récupérer fichier ========
fichier_infos = recuperer_fichier("./rue_de_londres.in")
